  ...
  return (
    ...
    >
      <motion.div
        ...
        onClick={onClose}
        className={cn(
          "absolute inset-0 bg-background/40 [backdrop-filter:blur(14px)_saturate(140%)] [-webkit-backdrop-filter:blur(14px)_saturate(140%)]",
          open ? "pointer-events-auto" : "pointer-events-none",
        )}
      />
      ...
      <div
        className={cn(
          ...
          placement === "bottom"
            ? "items-end pb-6 sm:items-center sm:pb-0"
            : "items-center",
        )}
      >
        <AnimatePresence initial={false}>
          {open ? (
            <motion.div
              ...
              className={cn(
                "pointer-events-auto relative w-full max-w-sm overflow-hidden rounded-3xl border border-border/70 bg-card/95 shadow-2xl backdrop-blur-xl will-change-transform",
                className,
              )}
            >
